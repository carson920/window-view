"""Tower 1: reflect the confirmed Tower 2 template, using the same ICP solver."""
import json, math, sys
from pathlib import Path
import numpy as np
ROOT=Path(__file__).resolve().parents[1]
tower=int(sys.argv[1]);assert tower in (1,2,3,4,5,6)
...
out=ROOT/f'data/sherwood-tower-{tower}';out.mkdir(exist_ok=True)
annotation={'templateSource':'data/chestwood-tower-2/annotations.json','placementMethod':f'User-confirmed Tower 2 template fitted to Tower {tower} official footprint; reflected={tower%2==1}','canvas':[1464,1696],'outline':outline,'rooms':rooms,'source':'https://i1.28hse.com/estate_data/108/4390/FLOOR/4390_20231121230203211831_large.jpg','layoutSource':('https://i1.28hse.com/estate_data/108/4390/FLOOR/4390_20231121230327817192_large.jpg' if tower%2 else 'https://i1.28hse.com/estate_data/108/4390/FLOOR/4390_20231121230332865318_large.jpg'),'sourceAuthority':'28Hse published plan; developer-original provenance not established','mapping':'Horizontal mirror of 1/3/5 plan: clockwise from southeast A B C D E F G H; lift bank above stair core in both.'}
(out/'annotations.json').write_text(json.dumps(annotation,indent=2))
official=json.loads((ROOT/f'data/kingswood-footprints/phase-2-tower-{tower}-official-footprint.json').read_text())
ring=np.array(official['geometry']['rings'][0]);origin,factor,q=en_frame(ring)
poly=np.array(outline)*[1,-1]
solutions=solve(sample(poly),sample(q),reflected=tower%2==1);best=solutions[0]
windows=[]
