...
### Window pattern samples
- Read `window-samples/window-pattern-samples.json` when present.
- These user-provided images are approved semantic references for pattern recognition.
- They can show examples of straight windows, projected bay windows, corner bay windows, multi-face bays, or non-window ledges.
- Never copy their coordinates into a tower plan.

### Supported representation classes
- `sill-line` — explicit visible straight glazing/sill line on source plan
- `bay-window-face` — source plan clearly shows a repeated projected bay-window module; trace the primary outward viewing face when a distinct sill line is not visible

### Overlay label style
- Overlay labels should be **English only**.
- Use short callouts such as `A living`, `B bed-1`, `C bay face`.
- Offset labels from the segment and use leader lines if needed.
- Avoid label overlap and avoid covering key source room text where practical.


## v6 bay / corner-window rules

- Recognize the **module first**, then choose the glazing/viewing face.
- `bay-window-face`: projected bay with one dominant outward face.
- `corner-bay-face`: angled/corner bay; store the primary outward face and optional secondary faces only if they represent separate view directions.
- `multi-face-bay`: multiple meaningful glazing faces may be separate windows/views.
- Never use the full bay perimeter, ledge perimeter, AC platform, or structural projection as a glazing segment.
- Approved positive/negative samples in `window-samples/` override generic visual heuristics.

## v6 overlay callouts

- English-only generated callouts.
- Use room-specific interior witness to prefer the outward side.
- Use leader lines.
- Collision-test label boxes; increase radial offset and stagger along the tangent until they do not overlap.
- Keep source-plan Chinese text untouched; do not add new mixed-language labels.
