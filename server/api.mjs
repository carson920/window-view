...
export const catalog = {
  ...
  estates: data.estates.filter(e => e.id.startsWith('kingswood-') || e.id === 'laguna-city').map(e => ({...pick(e, ['id','nameTC','nameEN','synthetic','defaultBuildingId']), buildings:e.buildings.map(b => ({...pick(b,['id','nameTC','nameEN','floors','floorModel']), flats:b.flats.map(f => pick(f,['id','label']))}))}))
};
...
const visibleEstate = id => id.startsWith('kingswood-') || id === 'laguna-city';
function outline(estate, building) {
 const file=estate.id==='laguna-city'?`laguna-${building.id}/official-footprint.json`:estate.id==='kingswood-chestwood'&&building.id==='tower-1'?'chestwood-tower-1-official-footprint.json':phases[estate.id]?`kingswood-footprints/phase-${phases[estate.id]}-${building.id}-official-footprint.json`:null;
