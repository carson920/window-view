...
const estate=read('data/properties.json').estates.find(e=>e.id==='kingswood-sherwood');
const source=read('data/chestwood-tower-2/result.json');
for(const tower of [1,2,3,4,5,6]) test(`Tower ${tower} retains confirmed template and uses independent official placement`,()=>{
 const r=read(`data/sherwood-tower-${tower}/result.json`);
 const o=read(`data/kingswood-footprints/phase-2-tower-${tower}-official-footprint.json`);
